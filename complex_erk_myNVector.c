...
#include <math.h>
#include <complex.h>
#include "nvector_serialcomplex.h" /* serial N_Vector types, fcts., macros */
#include <stdio.h>
...
int main(void)
{
  ...
  y = N_VNew_Mine(NEQ, ctx); /* Create serial vector for solution */
  if (check_flag((void*)y, "N_VNew_Serial", 0)) { return 1; }
  
double complex initial_condition = 1 + 2*I;

  MYNV_Ith(y, 0) = initial_condition; /* Specify initial condition */
  ...
  /* output initial condition to disk */
    fprintf(UFID," %.3"FSYM" | " "%.5"FSYM " + " "%.5" FSYM "i\n", T0, creal(MYNV_Ith(y, 0)), cimag(MYNV_Ith(y, 0)));
  ...
  while (Tf - t > 1.0e-15)
  {
    ...
    printf("  %10.6" FSYM "%10.6" FSYM " + " "%1.6" FSYM "i\n", t, creal(MYNV_Ith(y, 0)), cimag(MYNV_Ith(y, 0))); /* access/print solution */
    fprintf(UFID," %.3"FSYM" | " "%.5"FSYM " + " "%.5" FSYM "i\n", t, creal(MYNV_Ith(y, 0)), cimag(MYNV_Ith(y, 0)));
    if (flag >= 0)
  ...
  N_VDestroy_Mine(y);            /* Free y vector */
...
static int f(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  MYNV_Ith(ydot, 0) = t * MYNV_Ith(y, 0) + 2*I;
